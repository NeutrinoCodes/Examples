...
void main()
{
  ...
  for (j = j_min; j < j_max; j++)
  {
    ...
    a = vec4(P_mat*(V_mat*position_SSBO[n] + A));                               // Computing billboard boundary "a" (in clip space)...
    b = vec4(P_mat*(V_mat*position_SSBO[n] + B));                               // Computing billboard boundary "b" (in clip space)...
    ...
    e = vec4(P_mat*(V_mat*position_SSBO[n] + 0.5*(A + B)));                     // Computing billboard "ab" midpoint (in clip space)...
    ...
    color = color_SSBO[n];                                                      // Setting voxel color...  
