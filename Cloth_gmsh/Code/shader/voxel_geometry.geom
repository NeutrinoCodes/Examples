...
void main()
{
  ...
  for (j = j_min; j < j_max; j++)
  {
    ...
    base = s*vec2(-link.y, link.x);                                    // Computing segment orthogonal to PQ (in window space)...

    // COMPUTING BILLBOARD:
    A = vec4((-base.x - link.x)/AR, (-base.y - link.y), 0.0, 0.0);            // Computing offset point A (in clip space)...
    B = vec4((+base.x - link.x)/AR, (+base.y - link.y), 0.0, 0.0);            // Computing offset point B (in clip space)...
    C = vec4((-base.x + link.x)/AR, (-base.y + link.y), 0.0, 0.0);            // Computing offset point C (in clip space)...
    D = vec4((+base.x + link.x)/AR, (+base.y + link.y), 0.0, 0.0);            // Computing offset point D (in clip space)...
