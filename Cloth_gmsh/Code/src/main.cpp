...
int main ()
{
  ...
  //offset->data[0]    = 0;
  //neighbour_tuple.push_back (0);                                                                    // Assembling neighbour tuple...
  ...
  resting->init (neighbour_nodes);                                                                  // Initializing resiting position data...
  stiffness->init (neighbour_nodes);                                                                // Initializing stiffness data...
  ...
  for(i = 0; i < nodes; i++)
  {
    ...
    {
      k                  = offset->data[i] + j;                                                     // Setting neighbour tuple index...
      neighbour->data[k] = neighbour_tuple[k];                                                      // Setting neighbour tuple data...
      std::cout << "tuple = " << neighbour->data[k] << std::endl;
      resting->data[k]   = sqrt (
                                 pow (
                                      position->data[neighbour->data[k]].x - position->data[i].x,
                                      2
                                     ) +
                                 pow (
                                      position->data[neighbour->data[k]].y - position->data[i].y,
                                      2
                                     ) +
                                 pow (position->data[neighbour->data[k]].z - position->data[i].z, 2)
                                );
      stiffness->data[k] = K;
  ...
  K1->setarg (neighbour, 12);                                                                       // Setting neighbour kernel argument...
  ...
  K2->setarg (neighbour, 12);                                                                       // Setting neighbour kernel argument...
  ...
  Q->write (neighbour, 12);                                                                         // Writing neighbour on queue...
