                        ...
                        __global float*     dt_simulation)                      // Simulation time step [s].
{
        ...
        long n_F = neighbour_F[gid];                                            // Setting front neighbour index [#]...
        long n_L = neighbour_L[gid];                                            // Setting left neighbour index [#]...
        long n_D = neighbour_D[gid];                                            // Setting down neighbour index [#]...
        long n_B = neighbour_B[gid];                                            // Setting back neighbour index [#]...
        ...
        float4 P_F = position_int[n_F];                                         // Front neighbour position [m].
        float4 P_L = position_int[n_L];                                         // Left neighbour position [m].
        float4 P_D = position_int[n_D];                                         // Down neighbour position [m].
        float4 P_B = position_int[n_B];                                         // Back neighbour position [m].
        ...
        float4 resting_F = resting[n_F];                                        // Setting front neighbour resting position [m]...
        ...
        float4 resting_B = resting[n_B];                                        // Setting back neighbour resting position [m]...
        ...
        float4 k_F = stiffness[n_F];                                            // Setting front neighbour stiffness...
        float4 k_L = stiffness[n_L];                                            // Setting left neighbour stiffness...
        float4 k_D = stiffness[n_D];                                            // Setting down neighbour stiffness...
        float4 k_B = stiffness[n_B];                                            // Setting back neighbour stiffness...
        ...
        float4 D_F;                                                             // Front neighbour displacement [m]...
        float4 D_L;                                                             // Left neighbour displacement [m]...
        float4 D_D;                                                             // Down neighbour displacement [m]...
        float4 D_B;                                                             // Back neighbour displacement [m]...
        ...
        link_displacements(
                ...
                &D_B                                                            // Back neighbour displacement [m].
                );

        // COMPUTING NODE FORCE:
        Fnew = node_force  (
                k_R,                                                            // Right neighbour stiffness.
                k_U,                                                            // Up neighbour stiffness.
                k_F,                                                            // Front neighbour stiffness.
                k_L,                                                            // Left neighbour stiffness.
                k_D,                                                            // Down neighbour stiffness.
                k_B,                                                            // Back neighbour stiffness.
                D_R,                                                            // Right neighbour displacement [m].
                D_U,                                                            // Up neighbour displacement [m].
                D_F,                                                            // Front neighbour displacement [m].
                D_L,                                                            // Left neighbour displacement [m].
        ...
        assign_color(&col, &gravity);                                           // Assigning depth color [à]...
